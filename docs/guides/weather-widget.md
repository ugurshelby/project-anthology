Aşağıdaki component dış bir kaynaktan alındı ve özelleştirilerek sayfamızdaki asıl hava durumu componenti olmasını istiyorum. Çok gerekli yerlerde (bariz renk ve font farkı) gibi yerlerde tasarım değiştirilebilir, onun dışında birebir olarak alınmalı, sadece tasarımı değil backend ve api olarak da component tam olarak entegre ve çalışır halde olmalı.

...
import { getAllowedOrigin, corsHeaders } from '@/lib/f1Proxy';
import { logRequest } from '@/lib/apiLogger';

/**
 * F1 News aggregator (App Router port of legacy `api/news.ts`).
 * - Pulls RSS from a small list of high-signal F1 sources (parallel, per-source timeout).
 * - Filters to F1-only items, dedupes by canonical URL, clusters near-duplicate headlines.
 * - Sorts strictly by publication time (newest first). Never invents text.
 */
...
function processFeeds(rawItems: RawItem[]): NewsItem[] {
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const log = (status: number): void => logRequest({ route: 'news', method: 'GET', path, startedAt }, status);
  const cors = corsHeaders(getAllowedOrigin(req));

  // This endpoint takes no query parameters — reject any to keep the cache key clean.
  if ([...req.nextUrl.searchParams.keys()].length > 0) {
    log(400);
    return NextResponse.json({ error: 'Invalid request parameters' }, { status: 400, headers: cors });
  }
  ...
  try {
    ...
    if (msg.includes('timeout')) {
      log(504);
      return NextResponse.json({ error: 'Request timeout' }, { status: 504, headers: cors });
    }
    log(500);
    return NextResponse.json({ error: 'Failed to fetch news' }, { status: 500, headers: cors });
  }
}
